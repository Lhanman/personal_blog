    ...
    ContentContainer(widthStyle = ContentWidthStyle.Wide) {
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            if (state.featuredPosts.isNotEmpty()) {
                ...
                items(state.featuredPosts) { post ->
                    PostCard(
                        post = post,
            ...
            if (state.isLoading && state.featuredPosts.isEmpty() && state.recentPosts.isEmpty()) {
                item {
                    Box(Modifier.fillMaxWidth().padding(32.dp), contentAlignment = Alignment.Center) {
                        CircularProgressIndicator()
                    }
                }
            } else if (state.recentPosts.isEmpty() && !state.isLoading) {
