...
- [x] 7.5 在 `App()` 根组件中接入 `ThemeViewModel`，动态切换 `MaterialTheme`

## 8. 前端：导航与路由

- [x] 8.1 添加 `navigation-compose`（KMP 版本）依赖
- [x] 8.2 定义所有路由常量（`Screen` sealed class 或 object）
- [x] 8.3 实现 `AppNavHost`：注册所有页面路由（首页、文章、搜索、Tags、About、登录、后台）
- [x] 8.4 实现底部导航栏 / 侧边导航（移动端 vs 桌面端自适应布局）
...
- [x] 9.4 使用 Coil 3 加载封面图，实现加载占位符

## 10. 前端：文章阅读页

- [x] 10.1 添加 `multiplatform-markdown-renderer` 依赖
- [x] 10.2 实现 `BlogReaderViewModel`（加载文章详情）
- [x] 10.3 实现 `MarkdownContent` 组件（渲染 Markdown，代码块语法高亮）
- [x] 10.4 实现文章目录（TOC）侧边栏组件，支持点击平滑滚动
...
- [x] 12.4 实现 `TagPostsScreen`（某标签下的文章列表，复用 `PostCard`）

## 13. 前端：About 页与主题切换

- [x] 13.1 实现 `AboutScreen`（静态个人介绍内容，支持 Markdown 渲染）
- [x] 13.2 实现主题切换按钮组件（三态：日间/夜间/跟随系统）
- [x] 13.3 将主题切换按钮集成到顶部导航栏
...
- [x] 14.4 实现路由守卫：未登录访问需认证页面时重定向至登录页
...
- [x] 16.1 实现 `AdminViewModel`（文章列表管理、CRUD 操作）
- [x] 16.2 实现 `AdminPostListScreen`（文章管理列表，含编辑/删除操作）
- [x] 16.3 实现在线 Markdown 编辑器组件（左侧编辑、右侧预览双栏布局）
- [x] 16.4 实现标签选择器组件（最多选 5 个，超出时禁用添加）
- [x] 16.5 实现 `AdminPostEditScreen`（标题、内容编辑器、标签选择、发布/草稿切换）
- [x] 16.6 实现 `AdminUserListScreen`（用户列表，支持修改角色）
- [x] 16.7 后台管理路由添加 ADMIN 角色守卫
...
- [x] 18.1 后端：为 `PostRepository` 编写单元测试（使用 H2 内存数据库）
- [x] 18.2 后端：为主要 API 路由编写集成测试（`ktor-server-test-host`）
- [x] 18.3 前端：为 `BlogListViewModel` 编写单元测试（mock Repository）
- [x] 18.4 前端：为 `SearchViewModel` 编写单元测试（验证防抖逻辑）
